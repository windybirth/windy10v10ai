...
export class XNetTable {
   ...
   */
  SetTableValue<
    TName extends keyof XNetTableDefinitions,
    TStruct extends XNetTableDefinitions[TName],
    TKey extends keyof TStruct,
  >(tname: TName, key: TKey, value: TStruct[TKey]): void {
    if (!IsServer()) return;
  ...
  SetPlayerTableValue<
    ...
    TStruct extends XNetTableDefinitions[TName],
    TKey extends keyof TStruct,
  >(playerId: PlayerID, tname: TName, key: TKey, value: TStruct[TKey]): void {
    if (!IsServer()) return;
   ...
   * @memberof XNetTable
   */
  private _appendUpdateRequest<
    TName extends keyof XNetTableDefinitions,
    TStruct extends XNetTableDefinitions[TName],
    TKey extends keyof TStruct,
  >(playerId: PlayerID | undefined, tname: TName, key: TKey, value: TStruct[TKey]) {
    ...
    if (now == last_update_time) {
      print(
        `[XNetTable] ${mark_name}同一帧执行了多次更新，建议优化代码，一帧最多只更新一次，本次更新照常执行`,
      );
    }
  ...
  private _insertDataToQueue(
    data: string | XNetTableObject,
    playerId?: PlayerID,
    positively?: boolean,
  ) {
