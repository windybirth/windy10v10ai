...
local sDota2Version= '7.28'
local sDebugVersion= '20200716ver1.8b'
...
function J.IsOtherAllyCanKillTarget( bot, target )
	...
	do
		...
		if ally ~= nil
			...
			and ally:IsFacingLocation( target:GetLocation(), 30 )
			and GetUnitToUnitDistance( ally, target ) <= ally:GetAttackRange() + 50
		then
...
function J.GetInvUnitInLocCount( bot, nRadius, nFindRadius, loc, pierceImmune )
	...
	for _, u in pairs( unitList ) do
		if ( ( pierceImmune and J.CanCastOnMagicImmune( u ) )
			...
			and GetUnitToLocationDistance( u, loc ) <= nFindRadius
		then
			nUnits = nUnits + 1
		end
	end
...
function J.GetInLocLaneCreepCount( bot, nRadius, nFindRadius, loc )

	local nUnits = 0
	if nRadius > 1600 then nRadius = 1600 end
	local unitList = bot:GetNearbyLaneCreeps( nRadius, true )
	for _, u in pairs( unitList ) do
		if GetUnitToLocationDistance( u, loc ) <= nFindRadius
		then
			nUnits = nUnits + 1
		end
	end
...
function J.IsMoving( bot )
	...
	if GetUnitToLocationDistance( bot, loc ) > bot:GetCurrentMovementSpeed() * 0.45
	then
		return true
	end
...
J.GetInvUnitInLocCount( bot, nRadius, nFindRadius, loc, pierceImmune )
J.GetInLocLaneCreepCount( bot, nRadius, nFindRadius, loc )
