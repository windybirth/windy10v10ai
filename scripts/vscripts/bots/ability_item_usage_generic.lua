...
function X.GetNeedTangoMidLaner()
...
X.ConsiderItemDesire["item_glimmer_cape"] = function( hItem )
	...
	then
		...
		if ( J.IsRetreating( bot ) and bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_VERYHIGH )
			 or ( botTarget == nil and #hNearbyEnemyHeroList > 0 and J.GetHP( bot ) < 0.36 + ( 0.09 * #hNearbyEnemyHeroList ) )
		then
			hEffectTarget = bot
			sCastMotive = '自己撤退'
			return BOT_ACTION_DESIRE_HIGH, hEffectTarget, sCastType, sCastMotive
...
X.ConsiderItemDesire["item_quelling_blade"] = function( hItem )
	...
	if DotaTime() > lastQuellingBladeUseTime + 0.8
		and ( J.IsGoingOnSomeone( bot ) or J.IsFarming( bot ) )
	then
...
X.ConsiderItemDesire["item_tango"] = function( hItem )
	...
	if	DotaTime() > -88 and DotaTime() < 0 and bot:DistanceFromFountain() < 400
		...
		and tCharge >= 3
		and DotaTime() > J.Role['fLastGiveTangoTime'] + 2.0
	then
		local target = X.GetNeedTangoMidLaner()
		if target ~= nil
		then
...
function X.CanJuke()
	...
	do
		local enemyDamage = enemy:GetEstimatedDamageToTarget( false, bot, 3.5, DAMAGE_TYPE_ALL )
...
X.ConsiderItemDesire["item_tpscroll"] = function( hItem )
	...
	then
		local hNeedDropItem = J.Item.GetNeedDropNeutralItem( bot )
	...
	then
		...
		if bot:GetLevel() >= 22	and #nNearEnemyList == 0
			...
			and J.GetAroundTargetAllyHeroCount( nAncient, 1600 ) == 0
		then
